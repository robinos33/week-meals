...
--
-- Un `add column` par ordre : SQLite n'en accepte pas plusieurs à la fois.
alter table households add column onboarding_until     text;
alter table households add column onboarding_code_hash text;
alter table households add column onboarding_attempts  integer not null default 0;
alter table households add column onboarding_user_id   blob references users (id) on delete set null;
...
-- d'authentification — d'où un simple `text`, là où Postgres avait un `jsonb`
-- dont personne n'interrogeait la structure. `credential_id` en est extrait
-- pour l'unicité et les recherches. Les drapeaux `backup_*` sont dénormalisés
-- pour l'affichage.
